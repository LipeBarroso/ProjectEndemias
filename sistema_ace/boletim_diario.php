...
if (!empty($data_filtro)) {
  ...
  $where_data = " AND v.data = '{$conn->real_escape_string($data_filtro)}'";
}
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; margin:0; background:#f7f9fc; }
    .wrap { max-width:1000px; margin:28px auto; padding:18px; background:#fff; border-radius:10px; box-shadow:0 8px 24px rgba(0,0,0,0.06); }
    h2 { color:#006666; margin:0 0 12px 0; }
    table { width:100%; border-collapse:collapse; margin-top:12px; }
    th, td { padding:10px 8px; border-bottom:1px solid #eee; font-size:14px; text-align:left; }
    th { background:#f2f7f7; color:#006666; font-weight:700; }
    .empty { text-align:center; padding:20px; color:#666; }
    .btn { width:100%; padding:10px; border-radius:8px; border:none; font-size:14px; font-weight:bold; cursor:pointer; transition:background 0.3s, transform 0.2s; }
    .btn:hover { transform:translateY(-2px); }
    .btn-trabalhar { background:#009688; color:white; }
    .btn-trabalhar:hover { background:#00796b; }
    .btn-voltar { margin-top:20px; background:#1976d2; color:white; padding:12px; border-radius:8px; font-size:16px; font-weight:bold; cursor:pointer; width:100%; display:inline-block; text-align:center; text-decoration:none; transition:background 0.3s, transform 0.2s; border:none; }
    .btn-voltar:hover { background:#1565c0; transform:translateY(-2px); }
...
<body>
  <div class="wrap">
    ...
    <table>
      ...
      <tbody>
        <?php if (empty($rows)): ?>
          <tr><td class="empty" colspan="11">Nenhum imóvel cadastrado.</td></tr>
        <?php else: ?>
